...
RemoteSpy.Name = math.random()
local BG = Instance.new("Frame")
local cc = function (a) coroutine.wrap(a)()return end
...
Title.Text = "Remote spy H17S3 EDIT"
...
local namecall_script = function(object, method, ...)
	local script = "--// Remote spy by H17S3 EDIT\n"
	local args = {}
	for i, v in pairs{...} do
		script = script .. "local Arg_" .. i .. " = " .. (type(v) == "table" and Table_TS(v) or GetType(v)) .. "\n"
		table.insert(args, "Arg_" .. i)
	end
...
Decompile.MouseButton1Down:Connect(function()
	...
	repeat task.wait()
		...
		wait(.1)
		waited +=1
	until source or waited >10000
	--Decompiled with Synapse X Luau decompiler
	source = string.gsub(source, "Synapse X", "Synapse X and Remote2script edit by h17s3")
